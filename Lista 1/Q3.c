...
TAB* TAB_maior(TAB *a){
    ...
    TAB *resp = a;
    if((maior_esq) && (maior_esq->info > resp->info)) resp = maior_esq;
    if((maior_dir) && (maior_dir->info > resp->info)) resp = maior_dir;
    return resp;
}
...
int main(void){
  ...
  TAB_imp_ident(a);
  TAB * teste = TAB_maior(a);
  printf("O maior e -> %d", teste->info);
  printf("\n");
